...
def student_dashboard():
    ...
    for i , sub_node in enumerate(subjects):
        ...
        def unenroll_button():
            if st.button('Unenroll from this cource' , type='tertiary' , width='stretch' ,key=f"unenroll_{sub['subject_id']}", icon=':material/delete_forever:'):
                unenroll_student_to_subject(student_id , sid)
                st.toast(f'Unenrolled from {sub['name']} succesffully')
                st.rerun()
...
def student_screen():
    ...
    st.header("Loging using faceID" , text_alignment='center')
    st.space()
    st.space()
    ...
    if show_registration:
        with st.container(border=True):
            ...
            new_name = st.text_input("Enter your name" , placeholder='E.g. alok vishwakarma')
            ...
            try:
                audio_data = st.audio_input('Record a short phrase like I am present , My name is alok.')
            except Exception:
