...
def  teacher_dashboard():
    ...
    with tab2:
        ...
        if st.button('Manage Subjects' ,type = type2 ,  width='stretch' , icon=':material/bookmark:'):
...
def teacher_tab_take_attendence():
    ...
    if not subjects:
        st.warning('you havent any subject yet please create one to begin')
        return
    ...
    with col2:
        if st.button('Add Photoes' , type='primary' , icon=':material/photo_prints:' , width='stretch'):
            add_photos_dialog()
    ...
    with c2 :
        if st.button('Run Face Analysis' , width='stretch' , type='secondary' , icon=':material/analytics:' , disabled=not has_photos):
            with st.spinner('Deep Scanning class room photos.....'):
                all_detected_id = {}
...
def teacher_screen_login():
    ...
    st.space()
    st.space()
    teacher_username = st.text_input("Enter username" , placeholder='mukesh' )
...
def teacher_screen_register():
    ...
    st.space()
    teacher_username = st.text_input("Enter username" , placeholder='@mukesh' )
    teacher_name = st.text_input("Enter name" , placeholder='mukesh gupta')
